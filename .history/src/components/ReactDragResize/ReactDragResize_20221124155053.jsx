...
const ReactDragResize = memo(
  forwardRef((props, ref) => {
    ...
    useEffect(() => {
      ...
      const newParentWidth = props.parentW
        ? props.parentW
        : parentElement.clientWidth
      const newParentHeight = props.parentH
        ? props.parentH
      ...
      setRight(
        newParentWidth -
          (props.w === 'auto' ? parentElement.scrollWidth : props.w) -
          newLeft
      )
      setBottom(
        newParentHeight -
          (props.h === 'auto' ? parentElement.scrollHeight : props.h) -
          newTop
      )
      if (props.dragHandle) {
        ;[...parentElement.querySelectorAll(props.dragHandle)].forEach(
          (dragHandle) => {
      ...
      if (props.dragCancel) {
        ;[...parentElement.querySelectorAll(props.dragCancel)].forEach(
          (cancelHandle) => {
    ...
    function bodyDown(ev) {
      ...
      if (
        props.dragHandle &&
        target.getAttribute('data-drag-handle') !== _uid.toString()
      ) {
        return
      }
      ...
      if (
        props.dragCancel &&
        target.getAttribute('data-drag-cancel') === _uid.toString()
      ) {
        return
      }
    ...
    function move(ev) {
      ...
      const delta = {
        x:
          (dimensionsBeforeMoveSync.current.pointerX - pageX) /
          props.parentScaleX,
        y:
          (dimensionsBeforeMoveSync.current.pointerY - pageY) /
          props.parentScaleY
      }
    ...
    function up(ev) {
      if (stickDragSync.current || bodyDragSync.current) {
        ...
        // 冲突检测有开启，并且冲突了
        if (
          conflictResult.type === 'on' &&
          conflictResult.isConflict === true
        ) {
          setPosition({

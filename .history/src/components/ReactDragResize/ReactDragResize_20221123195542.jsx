...
const ReactDragResize = memo(forwardRef((props, ref) => {
  ...
  const [, setDimensionsBeforeMove, dimensionsBeforeMoveSync] = useSyncState({ pointerX: 0, pointerY: 0, left: 0, right: 0, top: 0, bottom: 0, width: 0, heihgt: 0 }, false)
  ...
  const { recordHistorySteps, recordCurrentSteps, getHistorySteps, historyStepsSync, setCurStep, curStepSync } = useHistorySteps(props.historyStepsLength)
  ...
  useWatch(props.z, (newVal, oldVal) => {
    if (newVal >= 0 || newVal === 'auto') {
      setZindex(newVal)
    }
  }, { immediate: true })
  useWatch(props.isActive, (newVal, oldVal) => {
    setActive(newVal)
  }, { immediate: true })
  useWatch(active, (newVal, oldVal) => {
     if (props.isActive) {
            // this.$emit('activated');
        } else {
  ...
  useWatch(props.parentW, (newVal, oldVal) => {
    setRight(newVal - width - left)
    setParentWidth(newVal)
  })
  useWatch(props.parentH, (newVal, oldVal) => {
  ...
  const rdrStick = useCallback((stick) => {
  ...
  }, [parentWidth, top, bottom])

  const sizeStyle = useMemo(() => {
    return {
      width: props.w == 'auto' ? 'auto' : width + 'px',
      height: props.h == 'auto' ? 'auto' : height + 'px'
    }
      ...
      if (props.aspectRatio) {
          ...
          if (maxWidth !== null) {
            if (maxHeihgt === null) {
              ...
              if (maxWidth / maxHeihgt > aspectFactor) {
  ...
  function bodyUp () {
    ...
    // 还原鼠标按下位置
    setDimensionsBeforeMove({ pointerX: 0, pointerY: 0, left: 0, right: 0, top: 0, bottom: 0, width: 0, heihgt: 0 })
  ...
  return (
    <ReactDragResizeWrapper
      ...
      onMouseDown={(ev) => { bodyDown(ev) }}
      onTouchStart={(ev) => { bodyDown(ev) }}
      onTouchEnd={(ev) => { up(ev) }}
            ...
            return (
              <div
                ...
                className={classnames([
                  ...
                  'rdr-stick',
                  'rdr-stick-' + stick, props.isResizable ? '' : 'not-resizable'])}
...
ReactDragResize.propTypes = {
  ...
  // 移动步数 >=0
  gridX (props, propName, componentName) {
  ...
  // 移动步数 >=0
  gridY (props, propName, componentName) {
  ...
  // 父级宽度 >=0
  parentW (props, propName, componentName) {
  ...
  // 父级高度 >=0
  parentH (props, propName, componentName) {
  ...
  // 最大宽度
  maxw (props, propName, componentName) {
    const val = props.maxw
    if (!((typeof val === 'string') ? val === 'auto' : val >= 0)) {
      return new Error('参数maxh值类型校验失败');
    }
  },
  maxh (props, propName, componentName) {
  ...
  x: PropTypes.number,
  y: PropTypes.number,
  z (props, propName, componentName) {
    const val = props.z
    if (!((typeof val === 'string') ? val === 'auto' : val >= 0)) {
  ...
  axis(props, propName, componentName) {
